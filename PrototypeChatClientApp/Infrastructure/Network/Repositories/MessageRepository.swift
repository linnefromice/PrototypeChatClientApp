    ...
    func fetchMessages(conversationId: String, userId: String, limit: Int) async throws -> [Message] {
        ...
        do {
            ...
            case .undocumented(statusCode: let code, let body):
                ...
                print("❌ [MessageRepository] fetchMessages failed - Status: \(code), Error: \(error), Message: \(errorMessage ?? "N/A")")
                throw error
            }
    ...
    func sendMessage(conversationId: String, senderUserId: String, text: String) async throws -> Message {
        ...
        do {
            ...
            case .undocumented(statusCode: let code, let body):
                ...
                let error = NetworkError.from(statusCode: code, message: errorMessage)
                print("❌ [MessageRepository] sendMessage failed - Status: \(code), Error: \(error), Message: \(errorMessage ?? "N/A")")
                throw error
            }
        } catch let error as NetworkError {
